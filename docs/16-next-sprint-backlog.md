...
- 미확인 결과 배지/리마인드(`B02`) - v1 완료(2026-03-01, 미확인 결과 기준 알림 발송/스킵 정책 + 회귀 테스트)
- 개인 루틴 캘린더(`B01`) - v1 완료(2026-03-01, Home 최근 8주 구매/결과확인 히스토리 카드 + 회귀 테스트)
- 주간 리포트 카드(`B03`) - v1 보강 완료(2026-03-01, 당첨률/결과확인 상태 지표 추가 + 회귀 테스트)
...
- 실기기 부재 대응 운영 트랙(`BM-001`~`BM-005`) - `BM-001` 완료(2026-03-03, `69` 주간 루틴/캘린더 고정), `BM-002` 완료(2026-03-03, `70` 주간 비교 템플릿), `BM-003` 완료(2026-03-03, `71` 장기화 리스크 기준), `BM-004` 완료(2026-03-03, `72` 즉시 실행 번들 명령/완료 판정), `BM-005` 완료(2026-03-03, `73` 문서 동기화 체크리스트)
...
- 실기기 PASS-배포 체인 자동화 트랙(`BO-001`~`BO-005`) - `BO-001` 완료(2026-03-03, `run-physical-pass-firebase-chain.sh`), `BO-002` 완료(2026-03-03, `78` 체인 리포트 템플릿/해석 가이드), `BO-003` 완료(2026-03-03, `79` 수동 실행 체크리스트), `BO-004` 완료(2026-03-03, `bo004_dry_run_only_rehearsal_local_2026-03-03.md`), `BO-005` 예정
...
- 추천 모델 후보 검토(`F03`) - v1 완료(2026-03-01, Generator 선호 패턴 추천 생성 + 과거 티켓 빈도 기반 번호 재구성 + 단위 테스트)
