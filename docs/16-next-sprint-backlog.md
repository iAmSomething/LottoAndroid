...
- 개인 루틴 캘린더(`B01`) - v1 완료(2026-03-01, Home 최근 8주 구매/결과확인 히스토리 카드 + 회귀 테스트)
- 목표 기반 알림(`B04`) - v1 완료(2026-03-01, 토요일 18시 이전 미등록 리마인드 + 등록완료 알림 스킵 정책)
- 저장 흐름 2탭 단축(`A01`) - v1 완료(2026-03-01, Generator 원탭 CTA `랜덤 생성 후 바로 저장` + 회귀 테스트)
- QR 실패 유형별 즉시 가이드(`A04`) - v1 완료(2026-02-27, 실패 코드 분류 + 가이드 타이틀/메시지 매핑 + 회귀 테스트)
- 공식 구매 페이지 리다이렉트 UX(`A05`) - v2 완료(2026-02-27, Home + Result + Settings 확장, 1회 안내/실패 fallback 공통)
- 외부 이동 1회 안내 모달(`A06`) - v1 완료(2026-03-01, 현재 시각 기반 구매 시간 안내 + 3개 화면 공통 반영 + 정책 테스트)
...
- 프리미엄 후보 기능 검토(`F02`) - v1 완료(2026-02-27, Settings 로컬 백업/복원 버튼 + `tickets_backup_latest.json` 생성/복원 + 단위 테스트)
