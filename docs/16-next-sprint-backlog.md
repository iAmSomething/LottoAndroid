...
- 프리미엄 후보 기능 검토(`F02`) - v17 완료(2026-03-04, Settings 로컬 백업/복원 + 주차별 구매/당첨 CSV 생성/공유 + CSV 회차 범위 필터(시작/끝) + 필터 무데이터 공유 차단 가드 + AI 프롬프트 요청 필터/실포함 회차 보정 안내(전체/이상/이하/범위 + 필터 반영) + 필터 충족률 문구(x/y회차, %) + 분석 신뢰 등급(높음/보통/낮음) + 누락 회차 경고/회차번호/출처별 게임 수/신뢰도/회차 범위/CSV 스키마 가이드 문구 포함 + 단위 테스트)
