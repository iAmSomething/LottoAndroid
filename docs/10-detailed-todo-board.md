...
- [x] C-011 QR 실패 유형별 즉시 가이드 v1(지원형식/정보누락/회차오류/번호오류 분류 + 안내 타이틀) + 회귀 테스트
...
- [x] K-021 워치→폰 핸드오프 딥링크 계약 안정화(`weeklylotto://open?route=`) + 라우팅 단위 테스트(`AppDeepLinkTest`)
...
- [!] P-004 실기기 증적 확보: Wear 소형/대형 2종에서 첫 렌더/스크롤/터치 검증 로그 수집 (실기기 미보유로 블로커, 최신 blocked 증적: `docs/assets/distribution/wear_p4_device_evidence_checkpoint_2026-03-03.md`)
...
## BI. 개인 루틴 캘린더(B01) v1 구현(2026-03-01 Cycle-58)
...
## BK. 미확인 결과 리마인드(B02) v1 구현(2026-03-01 Cycle-60)
...
- [ ] BM-001 실기기 부재 `blocked` 증적 주간 갱신 루틴 정의(`P-004`, `BK`) 및 실행 캘린더 고정
- [ ] BM-002 `run-all-physical-gates-when-ready.sh` 출력 요약 리포트 템플릿 추가(주간 비교용)
