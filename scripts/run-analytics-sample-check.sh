...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/run-analytics-sample-check.sh [--serial <adb-serial>] [--save-log <path>]
...
TEST_CLASSES="com.weeklylotto.app.HomeScreenSmokeTest,com.weeklylotto.app.WeeklySaveFlowInstrumentedTest,com.weeklylotto.app.StatsCtaInstrumentedTest"

echo "[INFO] Clear previous analytics logcat buffer ($SERIAL)"
adb -s "$SERIAL" logcat -c

echo "[INFO] Run analytics sample instrumentation tests"
...
echo "[INFO] Verify analytics event samples"
./scripts/verify-analytics-events.sh --profile stats-cta --log-file "$TMP_LOG_FILE"

rm -f "$TMP_LOG_FILE"
echo "[PASS] Analytics sample check completed."
